...
class DateConverter {
  ...
  static bool getIsSameMonth(DateTime nowTime, DateTime previousTime) {
    //The function first converts both DateTime objects to the local time zone by calling the toLocal() method on each.
    DateTime now = nowTime.toLocal();
    DateTime previous = previousTime.toLocal();
    //It then compares the day, month, and year properties of the two DateTime objects to determine if they refer to the same day.
    //If they do, the function returns true. If they do not, the function returns false.
    if (now.month == previous.month && now.year == previous.year) {
      return true;
    }
    return false;
  }
  ...
  static String getMonthAndYeer(DateTime dateTime) {
    DateTime now = DateTime.now();
    DateTime yesterday = now.subtract(const Duration(days: 1));
    DateTime localDateTime = dateTime.toLocal();
    String monthKey = DateFormat('MMMM').format(dateTime);
    String formattedDate = '${getnameMont(monthKey)}  ${dateTime.year}';
    return formattedDate;
  }

  static String getnameMont(monthKey) {
    String translatedMonth;
    switch (monthKey) {
      case 'January':
        translatedMonth = 'يناير';
        break;
      case 'February':
        translatedMonth = 'فبراير';
        break;
      case 'March':
        translatedMonth = 'مارس';
        break;
      case 'April':
        translatedMonth = 'أبريل';
        break;
      case 'May':
        translatedMonth = 'مايو';
        break;
      case 'June':
        translatedMonth = 'يونيو';
        break;
      case 'July':
        translatedMonth = 'يوليو';
        break;
      case 'August':
        translatedMonth = 'أغسطس';
        break;
      case 'September':
        translatedMonth = 'سبتمبر';
        break;
      case 'October':
        translatedMonth = 'أكتوبر';
        break;
      case 'November':
        translatedMonth = 'نوفمبر';
        break;
      case 'December':
        translatedMonth = 'ديسمبر';
        break;
      default:
        translatedMonth = monthKey;
    }
